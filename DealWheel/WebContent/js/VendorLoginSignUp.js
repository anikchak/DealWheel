...
function forgotPasswordMail(){
	...
	var emailValid = checkEmailAddress(email);
	if(emailValid){
		$("#vendorForgotErrorMsgSpan").text("Login Details sent to the email address");
		$("#login_error_forgot").show();
		$.post("TriggerEmail",
				{
		...
		$("#login_error_forgot").show();
		$("#vendorForgotErrorMsgSpan").text("Email address incorrect");
	}
}
